        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {query.isFetching && data.items.length === 0 ? (
              ...
              <EmptyState search={searchParams.get("search") ?? ""} />
            ) : (
      ...
      <td colSpan={7} className="px-4 py-16 text-center">
        <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
          <div className="flex size-12 items-center justify-center rounded-2xl bg-cream-100 text-espresso-700">
            {search ? <Search className="size-5" /> : <Package className="size-5" />}
          </div>
          ...
          <p className="text-sm text-fg-muted">
            {search
              ? `Nothing matches “${search}”. Try a different term or clear filters.`
